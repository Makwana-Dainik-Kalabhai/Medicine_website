...
<head>
    ...
    <title>Shipped Orders List</title>
    ...
    <div class="wrapper ">
        ...
        <div class="main-panel">
            ...
            <div class="content">
                <div class="card">
                    <div class="row">
                        <span class="mx-5 py-3" style="color: green;">
                            <h6>Shipped Orders List</h6>
                        </span>
                    </div>
                </div>

                <?php
                $sel = $conn->prepare("SELECT * FROM `orders` WHERE `status`='Shipped'");
                $sel->execute();
                $sel = $sel->fetchAll();
                $i = 1;

                foreach ($sel as $row) {
                    if ($row["description"] != "Your Order has been Shipped now.") {
                        $up = $conn->prepare("UPDATE `orders` SET `description`='Your Order has been Shipped now.' WHERE `order_id`='" . $row["order_id"] . "'");
                        $up->execute();
                    } ?>
                    ...
                    <div class="card px-3 py-0 my-5 overflow-hidden">
                        ...
                        <div class="row py-2 border-bottom">
                            ...
                            <div class="col-md-1"><a href="http://localhost/php/medicine_website/admin_panel/orders/order_details/shipped.php?order_id=<?php echo $row["order_id"]; ?>" class="btn btn-secondary">View</a></div>
                        </div>
                    </div>
                <?php $i++;
                } ?>
