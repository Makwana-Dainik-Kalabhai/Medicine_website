...
if (isset($_POST["update-category"])) {
    if (isset($_FILES["cat-img"])) {
        ...
        if ($_POST["category"] != null && $_FILES["cat-img"]["name"]!=null) {
        ...
        else if ($_POST["category"] != null && $_FILES["cat-img"]["name"]==null) {
            ...
            $update = $conn->prepare("UPDATE `products` SET `cat_img`='" . $_FILES["cat-img"]["name"] . "' WHERE `product_id`='" . $_SESSION["product_id"] . "'");
    ...
    //
        ...
        if ($_POST["category"] != null && $_POST["cat-img"]!=null) {
            $update = $conn->prepare("UPDATE `products` SET `category`='" . $_POST["category"] . "', `cat_img`='" . $_POST["cat-img"] . "' WHERE `product_id`='" . $_SESSION["product_id"] . "'");
        } //
        else if ($_POST["category"] != null && $_POST["cat-img"]==null) {
            $update = $conn->prepare("UPDATE `products` SET `category`='" . $_POST["category"] . "' WHERE `product_id`='" . $_SESSION["product_id"] . "'");
        } //
        else if ($_POST["category"] == null && $_POST["cat-img"]!=null) {
            $update = $conn->prepare("UPDATE `products` SET `cat_img`='" . $_POST["cat-img"] . "' WHERE `product_id`='" . $_SESSION["product_id"] . "'");
        }
        $update->execute();
    }
    $_SESSION["cat_suc"] = "Product Category Details are Updated Successfully";
